									...
									<div className="mt-3 grid gap-4 min-[1180px]:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
										<form className="space-y-3 border-t border-[var(--color-brand-line)] pt-3" onSubmit={handleSubmitMcpServer}>
											<div className="flex items-center justify-between gap-3">
												...
												{mcpEditingServerId ? (
													<button type="button" className="ui-button-secondary border px-3 py-1.5 text-[0.72rem] font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-500 cursor-pointer" onClick={() => {
														resetMcpForm();
											...
											<label className="block">
												...
												<input type="text" value={mcpName} onChange={(event) => setMcpName(event.target.value)} placeholder="filesystem" className="ui-field-line mt-1.5 w-full border-b bg-transparent px-0 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" autoComplete="off" spellCheck={false} />
											</label>
											...
											<div className="grid gap-4 min-[640px]:grid-cols-[minmax(0,1fr)_auto]">
												<label className="block">
													...
													<select value={mcpTransport} onChange={(event) => setMcpTransport(event.target.value as McpServerTransport)} className="ui-field-line mt-1.5 w-full border-b bg-transparent px-0 py-2 text-sm text-[#171717] outline-none">
											...
											{mcpTransport === "stdio" ? (
												<>
													<label className="block">
														...
														<input type="text" value={mcpCommand} onChange={(event) => setMcpCommand(event.target.value)} placeholder="npx -y @modelcontextprotocol/server-filesystem" className="ui-field-line mt-1.5 w-full border-b bg-transparent px-0 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" autoComplete="off" spellCheck={false} />
													</label>
													<label className="block">
														<span className="font-mono text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[#64748b]">Arguments</span>
														<textarea value={mcpArgs} onChange={(event) => setMcpArgs(event.target.value)} placeholder="One argument per line" className="ui-field-surface mt-1.5 min-h-24 w-full border px-3 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" spellCheck={false} />
													</label>
												</>
											) : (
												<label className="block">
													<span className="font-mono text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[#64748b]">Server URL</span>
													<input type="url" value={mcpUrl} onChange={(event) => setMcpUrl(event.target.value)} placeholder="https://example.com/mcp" className="ui-field-line mt-1.5 w-full border-b bg-transparent px-0 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" autoComplete="off" spellCheck={false} />
												</label>
											)}

											<label className="block">
												<span className="font-mono text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[#64748b]">Environment variables</span>
												<textarea value={mcpEnv} onChange={(event) => setMcpEnv(event.target.value)} placeholder="KEY=value" className="ui-field-surface mt-1.5 min-h-24 w-full border px-3 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" spellCheck={false} />
											</label>

											{mcpTransport !== "stdio" ? (
												<label className="block">
													<span className="font-mono text-[0.68rem] font-bold uppercase tracking-[0.12em] text-[#64748b]">Request headers</span>
													<textarea value={mcpHeaders} onChange={(event) => setMcpHeaders(event.target.value)} placeholder="Authorization=Bearer ..." className="ui-field-surface mt-1.5 min-h-24 w-full border px-3 py-2 text-sm text-[#171717] placeholder:text-slate-400 outline-none" spellCheck={false} />
												</label>
											) : null}

											<button type="submit" className="ui-button-primary w-full border px-4 py-2 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-500 disabled:cursor-not-allowed disabled:opacity-45 cursor-pointer" disabled={mcpActionServerId !== null}>
														...
														return (
															<li key={server.id} className={`border px-3 py-3 ${server.enabled ? "border-[var(--color-brand-line-strong)] bg-white" : "border-[var(--color-brand-line)] bg-[rgba(255,255,255,0.72)]"}`}>
																<div className="flex flex-wrap items-start justify-between gap-3">
																	...
																	<div className="flex flex-wrap items-center gap-2">
																		<button type="button" className="ui-button-secondary border px-3 py-1.5 text-[0.72rem] font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-500 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer" onClick={() => handleEditMcpServer(server)} disabled={mcpActionServerId !== null}>Edit</button>
																		<button type="button" className="ui-button-secondary border px-3 py-1.5 text-[0.72rem] font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-500 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer" onClick={() => { void handleToggleMcpServer(server); }} disabled={mcpActionServerId !== null}>{isBusy ? "Saving..." : server.enabled ? "Disable" : "Enable"}</button>
																		<button type="button" className="ui-button-secondary border px-3 py-1.5 text-[0.72rem] font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-500 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer" onClick={() => { void handleDeleteSelectedMcpServer(server.id); }} disabled={mcpActionServerId !== null}>{isBusy ? "Deleting..." : "Delete"}</button>
																	</div>
																</div>
